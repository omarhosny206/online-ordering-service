...
public class ProductService {
    ...
    public List<Seller> getSellers(int id) {
        ...
        List<UserProduct> userProducts = sellerProductRepository.findAllByProductId(id);

        if (userProducts == null)
            return new ArrayList<>();

        userProducts.stream().forEach(sellerProduct -> sellers.add(sellerProduct.getSeller()));
        return sellers;
    }
    ...
    public List<Double> getPrices(int id) {
        ...
        userProducts.stream().sorted((s1, s2) -> (int) (s1.getPrice() - s2.getPrice())).forEach(sellerProduct -> prices.add(sellerProduct.getPrice()));
        return prices;
    }

    public List<UserProduct> getSellersPrices(int id) {
